...
import {docUploadPicIcon} from '../../Assets/TabSvgs';
import {Back, DoctorImg, Star, Clock} from '../../Assets/icons';
import ImagePicker from 'react-native-image-crop-picker'; // Import the ImagePicker module
...
const requestStoragePermission = async () => {
  try {
    ...
    if (granted === PermissionsAndroid.RESULTS.GRANTED) {
      ...
      // Proceed with opening the image picker
      handleImageUpload();
    } else {
  ...
  const handleImageUpload = async () => {
   ...
   const handleSubmit = async () => {
     try {
      ...
      //  const imageRef = storageRef.child('doctorImages/' + name);
      ...
      //  const imageUrl = await imageRef.getDownloadURL();
       ...
       Alert.alert('Doctor data uploaded successfully');
     } catch (error) {
          ...
          <View style={{marginTop: '10%', marginHorizontal: '5%'}}>
            ...
            <View
              ...
              <TouchableOpacity onPress={handleImageUpload}>
                <SvgXml
                  xml={docUploadPicIcon}
                  style={{alignSelf: 'center', marginTop: 20}}></SvgXml>
                <Text
                  style={{
                    alignSelf: 'center',
                    marginTop: 20,
